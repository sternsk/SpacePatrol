...
/*
    ...
    public createCollisionPolygon(imageUrl: String){
        ...
        let vectors
        let satPolygon: SAT.Polygon 

         // need to load the image as an html Image to apply getOutline from 
        ...
        // Convert the points array to SAT.Vector objects
        if (polygon)
            vectors = polygon.map(point => new SAT.Vector(point.x, point.y).scale(scalingFactor));

        // Create the SAT.Polygon with the converted points
        satPolygon = new SAT.Polygon(new SAT.Vector(-25, -22), vectors);
            ...
            case "station01":
                ...
                case "station02":
                    ...
                    hmtlStation02Image.onload = () =>{
                        ...
                        // Convert the array of points to a string format required by the SVG polygon element
                        const pointsString = polygon02.map((point: {x: number; y: number}) => `${point.x},${point.y}`).join(" ");
...
// Function to convert an array of {x, y} to an SVG path string
function pointsToPathString(points: { x: number, y: number }[]): string {
    if (points.length === 0) return ''; // Handle empty array

    // Start with 'M' for Move to the first point
    let pathString = `M${points[0].x},${points[0].y}`;

    // Use 'L' for Line to each subsequent point
    for (let i = 1; i < points.length; i++) {
        pathString += ` L${points[i].x},${points[i].y}`;
    }

    // Optionally, close the path if needed (Z command)
    // pathString += ' Z';  // Uncomment this line if the path should be closed (looped)
